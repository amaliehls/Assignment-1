...
library(stringr);library(tidyverse);library(dplyr);library(lme4);library(caret);library(mlbench)
...
data.train = na.omit(Assignment1data)
data.test = final
...
```{r setup, include=FALSE}
model.MLU = lmer(CHI_MLU ~ VISIT * Diagnosis + (1+VISIT|ID), data=data.train, REML = FALSE)

folds = createFolds(data.train,, k=4)
